...
#Panel A
tmp <- data %>% 
  select(SampleNumber,Protists_Biomass,Total_Fungal_Biomass,Total_Bacteria_Biomass,Undifferentiated_Biomass) %>% 
  mutate(SampleNumber=as.character(SampleNumber)) %>% 
...
#Panel A1
tmp <- data %>% 
  select(SampleNumber,Protists_Biomass,
...
tmp <- data %>% #coarse
  ...
  mutate(Total_Biomass=rowSums(across(ends_with("Biomass"))),
         SampleNumber=as.character(SampleNumber) ) %>% 
  ...
  select(SampleNumber,Percent_Protists,Percent_Fungi,Percent_Bacteria,Percent_Undifferentiated) %>% 
  pivot_longer(!SampleNumber, names_to= "variable", values_to = 'percent') %>% 
...
tmp <- data %>%  #fine
  ...
  mutate(Percent_Protists = Protists_Biomass/Total_Biomass, 
         ...
         Percent_Undifferentiated = Undifferentiated_Biomass/Total_Biomass) %>% 
  select(SampleNumber,Percent_Protists,
...
rownames(mat) <- as.character(data$SampleNumber)
...
gdat$SampleNumber <- rownames(gdat)
gdat <- left_join(gdat, meta , by='SampleNumber')
...
tmp <- data %>% 
  ...
  pivot_longer(!c(SampleNumber, Fungi_to_Bacteria_Ratio), names_to= "variable", values_to = 'value') %>% 
  left_join(., meta, by="SampleNumber") %>% 
  mutate(SampleNumber=as.character(SampleNumber)) 
...
g5 <- ggplot(tmp)+
  ...
  geom_jitter(mapping=aes(x=Biocrust_Type_Coarse,y=value, shape=Substrate, color=Substrate), 
              height=0, width=0.2, size=2,stroke=1)+
...
g6 <- ggplot(tmp)+
  geom_violin(mapping=aes(x=Biocrust_Type_Coarse, y=value, fill=Biocrust_Type_Coarse),alpha=0.8)+
  geom_jitter(mapping=aes(x=Biocrust_Type_Coarse,y=value,shape=Substrate, color=Substrate), 
              height=0, width=0.2, alpha=0.7, stroke=1)+
  scale_fill_manual(values=crust_pal_coarse)+
  scale_color_manual(values = c('#7397EF',"#AE8A14"))+
  scale_shape_manual(values=c(0,1))+
  facet_wrap(~variable, nrow=1, scales='free')+
...
g23 <- ggplot(tmp)+
  ...
  geom_jitter(mapping=aes(x=Biocrust_Type_Coarse,y=percent, color=Substrate, shape=Substrate), 
              height=0, width=0.25, alpha=0.7,stroke=1)+
...
g24 <- ggplot(tmp)+
  geom_violin(mapping=aes(x=Biocrust_Type_Coarse, y=percent, fill=Biocrust_Type_Coarse))+
  geom_jitter(mapping=aes(x=Biocrust_Type_Coarse,y=percent, color=Substrate, shape=Substrate), 
              height=0, width=0.2, alpha=0.5, stroke=1)+
  scale_fill_manual(values=crust_pal_coarse, name="Coarse Biocrust Type")+
  scale_color_manual(values =  c('#7397EF',"#AE8A14"))+
  scale_shape_manual(values=c(0,1))+
