...
library(network)
library(vroom)
#library(phyloseq) #error, ignoring for now
library(vegan)
library(patchwork)
library(ggnewscale)
library(ggforce)
...
library(raster)
library(purrr)
library(USAboundaries) #error no package, installed USA.state.boundaries instead
library(USAboundariesData) #error, does not exist
library(ggspatial)
install.packages("ncdf4") #added
...
ext <- extent(c(xmin = -124, xmax = -110, 
                ymin = 31, ymax = 38))
...
```
## note 8.20.25: move over to 'AI map MM.R' script now
